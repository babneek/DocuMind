    ...
    def ask_document_question(self, question: str, filter_metadata: dict = None, pageindex_doc_id: str = None):
        ...
        if hits:
            ...
            for i, hit in enumerate(hits):
                print(f"[RAG] Chunk {i} (dist={hit.get('distance', '?'):.3f}): {hit['content'][:150]}...")
        ...
        print(f"[RAG] Generating answer from {len(unique_contexts)} context blocks ({len(context_str)} chars)")
